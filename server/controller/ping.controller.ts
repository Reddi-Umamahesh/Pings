...
export const createPing = async (req: ExtenedRequest, res: Response):Promise<void> => {
  try {
...
export const updatePing = async (req: ExtenedRequest, res: Response) => {
    try {
        ...
        if (userId && userId != ping.author.toString()) {
            res.status(403).json({
                sucess: false, message: "You are not the author"
            })
            return
        }
        ...
        await ping.save();
        res.status(200).json({ sucess: true, message: "Ping updated successfully" });
        return;
    } catch (error) {
        res.status(500).json({
            sucess: false,
...
export const deletePing = async (req:Request , res: Response) => {
    try {
        ...
        res.status(200).json({ sucess: true, message: "Ping deleted successfully" });
        return;
